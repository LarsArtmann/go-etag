...
|~~BuildFlow usage in go-etag~~ resolved — 2026-09-23: buildflow ruled go-etag not-covered (report 15:35 a1); the AGENTS manual gate is the canonical loop|~~`buildflow format` / dry-run execute fine (auto-detect); format findings reviewed~~|~~go-etag has **no `.buildflow.yml` and no pre-commit hook** — formally not a "covered" project; fleet-consistency call is open~~|~~Owner decision~~|~~S~~|
