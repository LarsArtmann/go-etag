...
2. ~~README/cache-policy guidance: a 304 response preserves ETag; callers own `Cache-Control`/`Vary`. Cross-link the guidance now living in httputil's README (added 2026-08-29).~~ done at `721cfbd` — README "Cache policy: who owns which headers" section (transport owns validators; callers own `Cache-Control`/`Vary`)
3. ~~Ordering note for chains: ETag must see the FINAL representation — place it inside (after) compression; document the recommended Chain position (httputil README carries the equivalent guidance for the deprecated adapter path).~~ done at `721cfbd` — README "Mind the middleware order" bullet (ETag beneath compression)
...
_**Status (2026-09-10 docs-health pass):** items 2-3 remain open and are tracked as TODO_LIST.md #9. **All items resolved as of 2026-09-11** — file archived (docs-health pass)._
