...
|~~5~~|~~Identify which session/tool initiated the 1.27.1 bump and why it bypassed a build gate~~|~~High~~|~~S~~|~~Cleanup~~| carried — daemon identified as the attacker (AGENTS records both incidents); precise mechanism in `TODO_LIST.md` #4 (M26)
...
|~~11~~|~~Decide OQ2: accept Alex's captures as permanent `client/testdata/` fixtures?~~|~~Medium~~|~~S~~|~~Decision~~| done — `ab1bcec` OQ2 (reply sent; fixtures wait until captures arrive — ROADMAP OQ2)
|~~12~~|~~Draft the Alex reply email (Age answer + no-store/freshening changes shipped)~~|~~Medium~~|~~S~~|~~Documentation~~| done — owner sent the reply (ROADMAP OQ2 resolution)
|~~13~~|~~If fixtures accepted: commit captures with a provenance README~~|~~Medium~~|~~S~~|~~Feature~~| resolved via OQ2 — reopen as a TODO_LIST item when the captures arrive (ROADMAP OQ2)
|~~14~~|~~Decide OQ3: add a tag-triggered Release workflow or document manual-only as policy~~|~~Medium~~|~~S~~|~~Decision~~| done — `ab1bcec` OQ3: `release.yml` added
|~~15~~|~~If workflow: create it, trigger on `v*` after CI green, test on the next tag~~|~~Medium~~|~~S~~|~~Feature~~| done — workflow live; triggers on the root `v*` tag (AGENTS Release Conventions)
...
|~~17~~|~~Decide OQ5: promote the ~25 open-low archive items or let them die~~|~~Low~~|~~S~~|~~Decision~~| done — `ab1bcec` OQ5: die with the archives
|~~18~~|~~Decide OQ4: affirm the FNV-64a `Strong` default as a standing decision~~|~~Low~~|~~S~~|~~Decision~~| done — `ab1bcec` OQ4: Strong default stands
|~~19~~|~~Design opt-in freshness-based serving (§4.2 `max-age`/`Expires`) — default stays accelerator~~|~~High~~|~~L~~|~~Feature~~| carried — `TODO_LIST.md` #4 (M22 design doc)
|~~20~~|~~Implement §4.2 freshness serving behind the opt-in~~|~~High~~|~~L~~|~~Feature~~| follows M22 — ROADMAP Theme 1
|~~21~~|~~Add §4.2 spec tests + `docs/rfc9111-conformance.md` updates~~|~~High~~|~~M~~|~~Quality~~| follows M22 — ROADMAP Theme 1
|~~22~~|~~Implement `stale-while-revalidate` on top of §4.2~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 1 (raw idea)
|~~23~~|~~Implement `stale-if-error` on top of §4.2~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 1 (raw idea)
|~~24~~|~~Add `Last-Modified`/`If-Modified-Since` as a second validator type~~|~~Medium~~|~~L~~|~~Feature~~| lives in ROADMAP Theme 1 (raw idea)
|~~25~~|~~Add client hooks `OnHit`/`OnStore`/`OnFreshen`/`OnInvalidate` (mirror server)~~|~~Medium~~|~~M~~|~~Feature~~| carried — `TODO_LIST.md` #4 (M23 spike)
|~~26~~|~~Per-host key partitioning by default (cross-origin KeyFunc safety)~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 3 (raw idea)
|~~27~~|~~Opt-in disk persistence of the client cache~~|~~Low~~|~~L~~|~~Feature~~| lives in ROADMAP Theme 3 (raw idea)
|~~28~~|~~Singleflight request coalescing for concurrent identical GETs~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 3 (raw idea)
|~~29~~|~~Project website launch (sibling Astro/Starlight pattern)~~|~~Medium~~|~~L~~|~~Documentation~~| lives in ROADMAP Theme 4
|~~30~~|~~Demo video for the website landing page~~|~~Low~~|~~L~~|~~Documentation~~| lives in ROADMAP Theme 4
|~~31~~|~~Comparison table vs other Go ETag/caching libraries~~|~~Medium~~|~~M~~|~~Documentation~~| carried — `TODO_LIST.md` #4 (M25)
|~~32~~|~~awesome-go listing submission~~|~~Low~~|~~S~~|~~Documentation~~| carried — `TODO_LIST.md` #4 (M25)
|~~33~~|~~OTEL/Prometheus wiring recipe on the existing hooks~~|~~Medium~~|~~S~~|~~Documentation~~| lives in ROADMAP Theme 4
|~~34~~|~~`go-etag/otel` sub-module — stays parked until hooks battle-tested AND consumer asks~~|~~Low~~|~~L~~|~~Feature~~| lives in ROADMAP Parked (demand-gated; unblock note added 2026-09-23)
|~~35~~|~~Root shim deletion — executes at v1.0.0 only~~|~~Low~~|~~S~~|~~Cleanup~~| lives in ROADMAP Parked + `AGENTS.md` v1.0.0 runbook seed
|~~36~~|~~Define v1.0.0 criteria (what besides shim deletion gates the major bump)~~|~~Medium~~|~~S~~|~~Documentation~~| carried — `TODO_LIST.md` #4 (M24)
...
|~~43~~|~~Record the five OQ decisions in ROADMAP once made (annotate, don't rewrite)~~|~~Medium~~|~~S~~|~~Documentation~~| done — `ab1bcec` recorded OQ2–OQ10 in `ROADMAP.md`
...
|~~47~~|~~Run `erraudit --no-suppress` on the next error-surface change~~|~~Low~~|~~S~~|~~Quality~~| dies per OQ5 — standing practice; revive on the next error-surface change
|~~48~~|~~Design a build-gate so go.mod commits cannot land red (hook or sweep rule)~~|~~Medium~~|~~M~~|~~Quality~~| superseded — release-gate guards + tag-only-after-CI doctrine cover the class
