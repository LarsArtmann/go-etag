...
// Code is a machine-readable error code in the etag error taxonomy,
// e.g. "http.etag_write_failed". Its prefix before the first dot is the
// Domain, identifying the failing component; the full code identifies the
// specific failure.
type Code string

// Domain identifies the failing component of an error code: the prefix
// before the first dot (e.g. "http" for "http.etag_write_failed"). Domains
...
// safely: WithContext, WithContextf, and friends clone rather than mutate.
...
//
//	if etag.InDomain(err, etag.Domain("http")) { ... }
