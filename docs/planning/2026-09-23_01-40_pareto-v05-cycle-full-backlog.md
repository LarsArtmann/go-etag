...
| M19 | **Process hardening:** numbered-item detector before archive moves (`grep -n '^\s*\d\+\.\s\*\*' <f> | grep -v '~~'`); `annotate-rows.py --dry-run` trial on one annotated file; before→after scoring rule for audit sessions — recorded in AGENTS.md where sessions read them | Medium     | Med    | S      | 45m    | rest    |
...
**Sequencing notes:** M1 → M2 is the spine; M8 and M9 land ideally *before* M2 (the
