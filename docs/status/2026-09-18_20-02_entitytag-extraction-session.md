...
|~~nolint removal on `ETag{}`~~|~~Lint is green with the directive removed~~|~~Root cause of why exhaustruct_v5 stopped flagging the alias zero-value is unknown; suppression is now absent if the situation changes~~|~~Investigation only~~|~~S~~| done — lint green on every gate since
...
|---|---|---|---|---|
|~~1~~|~~Resolve the local toolchain split: install go1.27.1 or set `GOTOOLCHAIN=auto` persistently (unblocks LSP, kills session noise)~~|~~High~~|~~S~~|~~Bug~~| done — `GOTOOLCHAIN=auto` command convention in `AGENTS.md`; LSP env fixed 2026-09-22
...
|~~3~~|~~Investigate why exhaustruct_v5 no longer flags `ETag{}` post-alias; record root cause in AGENTS.md, restore protection if needed~~|~~Medium~~|~~S~~|~~Quality~~| dies per OQ5 — lint is green without the directive; investigation revive on demand
...
|~~11~~|~~Add `Example` functions for the `entitytag` package (ParseETag round-trip, weak vs strong)~~|~~Low~~|~~S~~|~~Documentation~~| done — shipped v0.5.0 (`32c9793`: `ExampleParseETag`, `ExampleETag_weakVsStrong`, `ExampleParseETagList`)
|~~12~~|~~Decide OQ2: Alex reply email + `client/testdata/` fixtures~~|~~Medium~~|~~S~~|~~Decision~~| done — `ab1bcec` (resolutions in `ROADMAP.md` Open Questions)
|~~13~~|~~Decide OQ3: tag-triggered Release workflow vs documented manual-only~~|~~Medium~~|~~S~~|~~Decision~~| done — `ab1bcec` (resolutions in `ROADMAP.md` Open Questions)
|~~14~~|~~Decide OQ4: affirm FNV-64a `Strong` standing decision~~|~~Low~~|~~S~~|~~Decision~~| done — `ab1bcec` (resolutions in `ROADMAP.md` Open Questions)
|~~15~~|~~Decide OQ5: ~25 archive open-low items promote-or-die~~|~~Low~~|~~S~~|~~Decision~~| done — `ab1bcec` (resolutions in `ROADMAP.md` Open Questions)
|~~16~~|~~Decide OQ6: deprecated shim scope (typed surface in v0.3.x vs frozen)~~|~~Low~~|~~S~~|~~Decision~~| done — `ab1bcec` (resolutions in `ROADMAP.md` Open Questions)
|~~17~~|~~Decide OQ7: keep all six error-family constructors or trim~~|~~Low~~|~~S~~|~~Decision~~| done — `ab1bcec` (resolutions in `ROADMAP.md` Open Questions)
|~~18~~|~~Design opt-in freshness serving (§4.2 `max-age`/`Expires`) — default stays accelerator~~|~~High~~|~~L~~|~~Feature~~| carried — `TODO_LIST.md` #4 (M22 design doc)
|~~19~~|~~Implement §4.2 freshness serving behind the opt-in~~|~~High~~|~~L~~|~~Feature~~| lives in ROADMAP Theme 1 — implementation follows the M22 design doc
|~~20~~|~~§4.2 spec tests + rfc9111-conformance.md updates~~|~~High~~|~~M~~|~~Quality~~| lives in ROADMAP Theme 1 — rides the M22 design doc
|~~21~~|~~`stale-while-revalidate` support~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 1 (raw idea)
|~~22~~|~~`stale-if-error` support~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 1 (raw idea)
|~~23~~|~~`Last-Modified`/`If-Modified-Since` as second validator type~~|~~Medium~~|~~L~~|~~Feature~~| lives in ROADMAP Theme 1 (raw idea)
|~~24~~|~~Client hooks `OnHit`/`OnStore`/`OnFreshen`/`OnInvalidate`~~|~~Medium~~|~~M~~|~~Feature~~| carried — `TODO_LIST.md` #4 (M23 spike)
|~~25~~|~~Per-host key partitioning by default~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 3 (raw idea)
|~~26~~|~~Opt-in disk persistence~~|~~Low~~|~~L~~|~~Feature~~| lives in ROADMAP Theme 3 (raw idea)
|~~27~~|~~Singleflight request coalescing~~|~~Medium~~|~~M~~|~~Feature~~| lives in ROADMAP Theme 3 (raw idea)
|~~28~~|~~Project website launch (Astro/Starlight sibling pattern)~~|~~Medium~~|~~L~~|~~Documentation~~| lives in ROADMAP Theme 4
|~~29~~|~~Demo video for the website~~|~~Low~~|~~L~~|~~Documentation~~| lives in ROADMAP Theme 4
|~~30~~|~~Comparison table vs other Go ETag/caching libraries~~|~~Medium~~|~~M~~|~~Documentation~~| carried — `TODO_LIST.md` #4 (M25)
|~~31~~|~~awesome-go submission~~|~~Low~~|~~S~~|~~Documentation~~| carried — `TODO_LIST.md` #4 (M25)
|~~32~~|~~OTEL/Prometheus wiring recipe on existing hooks~~|~~Medium~~|~~S~~|~~Documentation~~| lives in ROADMAP Theme 4 (recipe on existing hooks)
|~~33~~|~~`go-etag/otel` sub-module (stays demand-gated)~~|~~Low~~|~~L~~|~~Feature~~| lives in ROADMAP Parked (demand-gated; unblock note added 2026-09-23)
|~~34~~|~~Root shim deletion at v1.0.0~~|~~Low~~|~~S~~|~~Cleanup~~| lives in ROADMAP Parked + `AGENTS.md` v1.0.0 runbook seed
|~~35~~|~~Define v1.0.0 criteria~~|~~Medium~~|~~S~~|~~Documentation~~| carried — `TODO_LIST.md` #4 (M24)
|~~36~~|~~Extend client fuzz coverage to Cache-Control directive variants~~|~~Medium~~|~~M~~|~~Quality~~| carried — `TODO_LIST.md` #1 (M16)
|~~37~~|~~Design a build-gate so go.mod commits cannot land red (hook/sweep rule)~~|~~Medium~~|~~M~~|~~Quality~~| superseded — release-gate guards (go.work tracked assert, directive parity) + tag-only-after-CI doctrine cover the class
|~~38~~|~~Concurrent-session convention (announce file set; or session locks)~~|~~Medium~~|~~S~~|~~Process~~| done 2026-09-23 — `AGENTS.md` shared-tree protocol bullet
|~~39~~|~~Benchmark A/B: codify the interleave + allocs-equality protocol in AGENTS.md benchmark discipline~~|~~Low~~|~~S~~|~~Process~~| dies per OQ5 — revive on demand (ROADMAP OQ5)
|~~40~~|~~Post-change doc checklist (six living docs) to stop FEATURES/README drift~~|~~Medium~~|~~S~~|~~Process~~| dies per OQ5 — the docs-health skill owns this loop
...
|~~42~~|~~`Strength.String()` method (archive item) if OQ5 promotes it~~|~~Low~~|~~S~~|~~Feature~~| done — OQ5 resolved: archive items die with the archives (ROADMAP OQ5)
|~~43~~|~~obs-text validation in the parser (archive item) if OQ5 promotes it~~|~~Low~~|~~S~~|~~Feature~~| done — OQ5 resolved: archive items die with the archives (ROADMAP OQ5)
|~~44~~|~~Single-pass `ParseETagList` (archive item) if OQ5 promotes it~~|~~Low~~|~~S~~|~~Feature~~| done — OQ5 resolved: archive items die with the archives (ROADMAP OQ5)
|~~45~~|~~Isolated parser benchmarks (archive item) if OQ5 promotes it~~|~~Low~~|~~M~~|~~Quality~~| done — OQ5 resolved: archive items die with the archives (ROADMAP OQ5)
...
|~~47~~|~~Re-run full fuzz jobs at CI duration (not 8s smoke) before the next tag~~|~~Medium~~|~~M~~|~~Quality~~| done — CI fuzz green through the v0.6.0 tag run (report 16:40 a1)
...
|~~50~~|~~Consumer sweep after v0.3.2: httputil/cqrs-htmx version surfaces + workspace both-modes verify — still open for v0.4.0 (TODO_LIST #3, 2026-09-23)~~|~~High~~|~~M~~|~~Quality~~| done — superseded by the v0.6.0 six-consumer sweep (report 16:40 a4)
