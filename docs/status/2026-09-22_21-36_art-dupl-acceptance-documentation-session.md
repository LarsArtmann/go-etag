...
| 2  | HARVEST this report's list into TODO_LIST.md / ROADMAP.md (docs-health HARVEST) so it does not die in this timestamped file                   | High   | S      | Documentation |
| 3  | Look at the 1-line `go.mod` change inside daemon commit `969d077` (not authored this session)                                                 | Medium | S      | Cleanup       |
| 4  | Date-stamp the testutil comment's "single accepted clone group" claim (e.4)                                                                   | Low    | S      | Documentation |
| 5  | Add `GOTOOLCHAIN=auto` prefix into the AGENTS.md command block's golangci-lint lines (prose note exists; block still shows bare commands)     | Low    | S      | Documentation |
| 6  | Pin HEAD-freshening marker-stripping behavior with a spec test (carried from 09-11 06:18 report, partial item 1)                              | Medium | M      | Quality       |
...
| 12 | Check CI's golangci-lint job pins go 1.27.1 / GOTOOLCHAIN correctly (not inspected this session)                                              | Medium | S      | Quality       |
...
| 14 | Re-run `erraudit nolint-audit .` to re-confirm the two `drainAndClose` directives are still NEEDED (last verified 09-11)                       | Low    | S      | Quality       |
...
| 18 | Run docs-health VERIFY on `docs/rfc9111-conformance.md` against `client/spec_test.go` claims                                                  | Medium | M      | Documentation |
| 19 | docs-health VERIFY pass over AGENTS.md claims touched by recent refactors (error tables, gotchas)                                             | Medium | M      | Documentation |
...
| 34 | Verify docs/DOMAIN_LANGUAGE.md exists / is current for this repo (global project-discovery checklist)                                         | Low    | S      | Documentation |
...
| 44 | Add a short "verification gate" checklist to AGENTS.md Commands (fmt + lint + race + vet + art-dupl) so first-pass scope is unambiguous       | Medium | S      | Documentation |
...
| 47 | Check FEATURES.md freshness (docs-health) — last touched before this session                                                                  | Low    | S      | Documentation |
