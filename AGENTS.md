...
Everything else carries over: tag only after CI green on the exact commit, load the go-release skill, pkg.go.dev is not a gate. The GitHub Release page is automated since OQ3 (2026-09-23): pushing the ROOT `v*` tag triggers `.github/workflows/release.yml`, which extracts the matching CHANGELOG section as the notes (it fails loudly if the section was not cut) and creates the Release as Latest, idempotently; nested tags get no Release pages. The proposal doc `docs/modularization/2026-09-23_modularization-proposal.html` records why metrics got a module (tombstone purity, not composability) and why go.mod replace directives stay banned (proxy-published repo).
