...
1. README: document `New(DefaultETagConfig())` as the minimal usage example (found missing during consumer verification).
...
4. Compliance-suite ownership: confirm whether the RFC 7232 compliance suite lives in this repo (httputil's 23-33 report moved items 10-17 here) and note that in `docs/`.
5. `deprecated.go`: confirm the `Deprecated:` doc comments carry `// Deprecated:` prefixes so gopals/staticcheck flag call sites.
