...
|~~5~~ carried — daemon identified as the attacker (AGENTS records both incidents); precise mechanism in `TODO_LIST.md` #4 (M26)|~~Identify which session/tool initiated the 1.27.1 bump and why it bypassed a build gate~~|~~High~~|~~S~~|~~Cleanup~~|
...
|~~11~~ done — `ab1bcec` OQ2 (reply sent; fixtures wait until captures arrive — ROADMAP OQ2)|~~Decide OQ2: accept Alex's captures as permanent `client/testdata/` fixtures?~~|~~Medium~~|~~S~~|~~Decision~~|
|~~12~~ done — owner sent the reply (ROADMAP OQ2 resolution)|~~Draft the Alex reply email (Age answer + no-store/freshening changes shipped)~~|~~Medium~~|~~S~~|~~Documentation~~|
|~~13~~ resolved via OQ2 — reopen as a TODO_LIST item when the captures arrive (ROADMAP OQ2)|~~If fixtures accepted: commit captures with a provenance README~~|~~Medium~~|~~S~~|~~Feature~~|
|~~14~~ done — `ab1bcec` OQ3: `release.yml` added|~~Decide OQ3: add a tag-triggered Release workflow or document manual-only as policy~~|~~Medium~~|~~S~~|~~Decision~~|
|~~15~~ done — workflow live; triggers on the root `v*` tag (AGENTS Release Conventions)|~~If workflow: create it, trigger on `v*` after CI green, test on the next tag~~|~~Medium~~|~~S~~|~~Feature~~|
...
|~~17~~ done — `ab1bcec` OQ5: die with the archives|~~Decide OQ5: promote the ~25 open-low archive items or let them die~~|~~Low~~|~~S~~|~~Decision~~|
|~~18~~ done — `ab1bcec` OQ4: Strong default stands|~~Decide OQ4: affirm the FNV-64a `Strong` default as a standing decision~~|~~Low~~|~~S~~|~~Decision~~|
|~~19~~ carried — `TODO_LIST.md` #4 (M22 design doc)|~~Design opt-in freshness-based serving (§4.2 `max-age`/`Expires`) — default stays accelerator~~|~~High~~|~~L~~|~~Feature~~|
|~~20~~ follows M22 — ROADMAP Theme 1|~~Implement §4.2 freshness serving behind the opt-in~~|~~High~~|~~L~~|~~Feature~~|
|~~21~~ follows M22 — ROADMAP Theme 1|~~Add §4.2 spec tests + `docs/rfc9111-conformance.md` updates~~|~~High~~|~~M~~|~~Quality~~|
|~~22~~ lives in ROADMAP Theme 1 (raw idea)|~~Implement `stale-while-revalidate` on top of §4.2~~|~~Medium~~|~~M~~|~~Feature~~|
|~~23~~ lives in ROADMAP Theme 1 (raw idea)|~~Implement `stale-if-error` on top of §4.2~~|~~Medium~~|~~M~~|~~Feature~~|
|~~24~~ lives in ROADMAP Theme 1 (raw idea)|~~Add `Last-Modified`/`If-Modified-Since` as a second validator type~~|~~Medium~~|~~L~~|~~Feature~~|
|~~25~~ carried — `TODO_LIST.md` #4 (M23 spike)|~~Add client hooks `OnHit`/`OnStore`/`OnFreshen`/`OnInvalidate` (mirror server)~~|~~Medium~~|~~M~~|~~Feature~~|
|~~26~~ lives in ROADMAP Theme 3 (raw idea)|~~Per-host key partitioning by default (cross-origin KeyFunc safety)~~|~~Medium~~|~~M~~|~~Feature~~|
|~~27~~ lives in ROADMAP Theme 3 (raw idea)|~~Opt-in disk persistence of the client cache~~|~~Low~~|~~L~~|~~Feature~~|
|~~28~~ lives in ROADMAP Theme 3 (raw idea)|~~Singleflight request coalescing for concurrent identical GETs~~|~~Medium~~|~~M~~|~~Feature~~|
|~~29~~ lives in ROADMAP Theme 4|~~Project website launch (sibling Astro/Starlight pattern)~~|~~Medium~~|~~L~~|~~Documentation~~|
|~~30~~ lives in ROADMAP Theme 4|~~Demo video for the website landing page~~|~~Low~~|~~L~~|~~Documentation~~|
|~~31~~ carried — `TODO_LIST.md` #4 (M25)|~~Comparison table vs other Go ETag/caching libraries~~|~~Medium~~|~~M~~|~~Documentation~~|
|~~32~~ carried — `TODO_LIST.md` #4 (M25)|~~awesome-go listing submission~~|~~Low~~|~~S~~|~~Documentation~~|
|~~33~~ lives in ROADMAP Theme 4|~~OTEL/Prometheus wiring recipe on the existing hooks~~|~~Medium~~|~~S~~|~~Documentation~~|
|~~34~~ lives in ROADMAP Parked (demand-gated; unblock note added 2026-09-23)|~~`go-etag/otel` sub-module — stays parked until hooks battle-tested AND consumer asks~~|~~Low~~|~~L~~|~~Feature~~|
|~~35~~ lives in ROADMAP Parked + `AGENTS.md` v1.0.0 runbook seed|~~Root shim deletion — executes at v1.0.0 only~~|~~Low~~|~~S~~|~~Cleanup~~|
|~~36~~ carried — `TODO_LIST.md` #4 (M24)|~~Define v1.0.0 criteria (what besides shim deletion gates the major bump)~~|~~Medium~~|~~S~~|~~Documentation~~|
...
|~~43~~ done — `ab1bcec` recorded OQ2–OQ10 in `ROADMAP.md`|~~Record the five OQ decisions in ROADMAP once made (annotate, don't rewrite)~~|~~Medium~~|~~S~~|~~Documentation~~|
...
|~~50~~ carried — `TODO_LIST.md` #2 (M16 Cache-Control corpus)|~~Extend client fuzz coverage beyond `FuzzHasNoStoreDirective` (Cache-Control variants)~~|~~Medium~~|~~M~~|~~Quality~~|
