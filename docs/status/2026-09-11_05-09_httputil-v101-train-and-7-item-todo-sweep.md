...
|~~2~~|~~`nix build` / `nix flake check` in cqrs-htmx BEFORE the push — vendorHash drift risk after 40+ module-graph changes~~|~~Critical~~|~~M~~|~~Bug~~| done — push executed and train-verified in-session (§a.1)
|~~3~~|~~Write the cqrs-htmx status report for the sweep (repo convention; documents baseline, the 3 pre-existing failures, repairs)~~|~~High~~|~~S~~|~~Documentation~~| Won't implement here — foreign repo (cqrs-htmx's own convention)
...
|~~6~~|~~Record in httputil `docs/RELEASE.md`: two independent lint pins (ci.yml + release.yml); changelog-link check belongs pre-push~~|~~High~~|~~XS~~|~~Documentation~~| Won't implement here — foreign repo (httputil's backlog)
...
|~~8~~|~~Verify pkg.go.dev rendered for `httputil@v1.0.1` and `server_timing@v1.0.1` (404 at trigger time; non-blocking)~~|~~Medium~~|~~XS~~|~~Release~~| Won't implement here — foreign repo (httputil's verification)
|~~9~~|~~Decide + execute: go-cqrs-lite `badgerengine` v4.2.1 (last train lag) — or hand to the go-cqrs-lite train owner~~|~~Medium~~|~~S~~|~~Feature~~| Won't implement here — foreign repo (go-cqrs-lite train)
|~~10~~|~~AGENTS.md updates: cqrs-htmx (train checker = enumeration authority), httputil (`go work vendor` is the vendoring tool)~~|~~Medium~~|~~XS~~|~~Maintenance~~| Won't implement here — foreign repos' AGENTS files
|~~11~~|~~Curate the v1.0.0 GitHub Release page (currently `--generate-notes` raw output; house style is curated)~~|~~Low~~|~~XS~~|~~Release~~| Won't implement here — foreign repo (httputil's Release page)
|~~12~~|~~benchstat via sanctioned path; regenerate the three baselines as a comparison table (pre-existing)~~|~~Medium~~|~~S~~|~~Quality~~| carried — `TODO_LIST.md` #1 (M11)
|~~13~~|~~Record accepted coverage floor (98.3%) as explicit decision (pre-existing)~~|~~Medium~~|~~XS~~|~~Quality~~| carried — `TODO_LIST.md` #2 (M13)
|~~14~~|~~`Freshened`/`Invalidated` counters on `Stats` (pre-existing)~~|~~Medium~~|~~S~~|~~Feature~~| carried — `TODO_LIST.md` #4 (M23 client hooks)
|~~15~~|~~Explicit subtest: caller-conditional HEAD receiving 304 passes through (pre-existing)~~|~~Medium~~|~~S~~|~~Quality~~| carried — `TODO_LIST.md` #1 (M17 spec pin-ups)
|~~16~~|~~HEAD-freshening × `resp.Uncompressed` interplay test (pre-existing)~~|~~Medium~~|~~S~~|~~Quality~~| carried — `TODO_LIST.md` #1 (M17, listed)
|~~17~~|~~GoDoc examples for `FreshenPerRFC`/`FreshenNone` (or one policy-selection example) — FreshenPolicy coverage is partial~~|~~Low~~|~~S~~|~~Documentation~~| done — `ExampleFreshenPerRFC`/`ExampleFreshenNone` shipped v0.5.0 (`32c9793`, CHANGELOG)
|~~18~~|~~Rebase/fix the red dependabot actions-group PR in httputil (was red pre-session; lint-pin fixes may unblock it)~~|~~Low~~|~~S~~|~~Maintenance~~| Won't implement here — foreign repo (httputil's dependabot PR)
|~~19~~|~~go directive floor policy: cqrs-htmx modules sit at `go 1.26.7` (patch floor) vs the skill's major.minor rule — decide once~~|~~Low~~|~~S~~|~~Policy~~| carried — `TODO_LIST.md` #4 (M27 cqrs-htmx toolchain pin)
|~~20~~|~~Consider `GOTOOLCHAIN` job-scoping audit for httputil/cqrs-htmx CI (same hazard go-etag just fixed)~~|~~Low~~|~~S~~|~~Bug~~| Won't implement here — foreign repos' CI (go-etag fixed its own)
|~~21~~|~~cqrs-htmx: verify `go.work.sum` cleanliness post-churn (currently pass-by-build; never explicitly inspected)~~|~~Low~~|~~XS~~|~~Maintenance~~| Won't implement here — foreign repo (cqrs-htmx's own hygiene)
| 22 | go-etag `transport_test.go:819` gopls SA1008 (`x-b` non-canonical key) — pre-existing warning, confirmed still present, never touched                                                                                     | Low      | XS     | Quality       |
