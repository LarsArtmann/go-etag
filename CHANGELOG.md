...
- GoDoc examples for the freshening policy constructors (client): `ExampleFreshenPerRFC` (the §4.3.4 default — every revalidation-provided field replaces the stored one) and `ExampleFreshenNone` (freshening off — every stored field survives the rebuild), mirroring `ExampleFreshenPolicy`.
- `BenchmarkTransportHeadFreshening` (client): isolates the RFC 9111 §4.3.5 freshening path so the 2026-09-11 dedup's cost is measured, not assumed — +432 B/op and +3 allocs/op per freshening HEAD against the pre-dedup arm (`reports/bench/2026-09-23_{before,after}-head-freshening.txt`, captured in git worktrees); the stored-validator change's missing before-state is filed alongside (`reports/bench/2026-09-23_before-typed-code-stored-validator.txt`).
- `scripts/pre-release-check.sh`: the full local release gate as one command — build, vet, race, lint, fmt-diff, type-aware erraudit (with the `[feature:logger]` filter), nolint-audit, `go mod verify`, replace/pseudo-version bans, and go-directive ↔ CI-pin parity (the check that catches silent go.mod relaxations).
