...
|~~M4~~ done — shipped with v0.5.0 (`af2b6d1`; report 04-52 §b)|~~**LSP fix (OWNER):** set `GOTOOLCHAIN=auto` in the Crush launcher for gopls + golangci-lint-ls; verify diagnostics load; do NOT touch the persisted global go env~~|~~High~~|~~High~~|~~S~~|~~30m~~|~~4%~~|
|~~M5~~ done — shipped with v0.5.0 (`af2b6d1`; report 04-52 §b)|~~**HEAD marker spec pin:** spec test that HEAD-freshened entries never persist the `FromCacheHeader` marker (the 06-18 dedup's incidental invariant); grep `spec_test.go` for existing coverage first; add the negative subtest~~|~~High~~|~~Med~~|~~S~~|~~45m~~|~~20%~~|
|~~M6~~ done — shipped with v0.5.0 (`af2b6d1`; report 04-52 §b)|~~**`cloneHeader` prove-or-delete:** prove `entry.header` can(n't) be nil → unit test the branch or delete the guard; coverage check on `persistFreshened`/`cloneHeader` (`go test -coverprofile`)~~|~~High~~|~~Med~~|~~S~~|~~45m~~|~~20%~~|
|~~M7~~ done — shipped with v0.5.0 (`af2b6d1`; report 04-52 §b)|~~**Benchmark pairs:** HEAD-path `header.Clone()` vs the git-reachable pre-dedup baseline; stored-validator before-state from a `07fe65c^` worktree; file both pairs under `reports/bench/` (`-benchmem -count=6`, interleaved arms)~~|~~Medium~~|~~Med~~|~~M~~|~~60m~~|~~20%~~|
|~~M8~~ done — shipped with v0.5.0 (`af2b6d1`; report 04-52 §b)|~~**`scripts/pre-release-check.sh`:** encode the full gate (build, vet, `-race`, lint, erraudit, `nolint-audit .`, `go mod verify`, replace/pseudo check, GOTOOLCHAIN pin); wire into AGENTS.md release conventions; dry-run on master~~|~~High~~|~~High~~|~~M~~|~~60m~~|~~20%~~|
|~~M9~~ done — shipped with v0.5.0 (`af2b6d1`; report 04-52 §b)|~~**GoDoc examples backfill:** entitytag (`ExampleParseETag` round-trip, weak-vs-strong comparison, `ParseETagList`), `FreshenPerRFC`, `FreshenNone`; all with `// Output:` (testableexamples)~~|~~Medium~~|~~Med~~|~~M~~|~~60m~~|~~20%~~|
|~~M10~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**CHANGELOG hygiene:** compare-link casing audit (`Larsartmann` vs `larsartmann`), a link-lint asserting every link resolves; fix findings~~|~~Low~~|~~Low~~|~~S~~|~~30m~~|~~rest~~|
|~~M11~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**benchstat:** install via sanctioned path; regenerate the four `reports/bench` baselines as comparison tables~~|~~Low~~|~~Med~~|~~S~~|~~45m~~|~~rest~~|
|~~M12~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**erraudit-in-CI (OWNER decision + wiring):** blocking default-mode job (0 violations invariant) + informational `nolint-audit`; `[feature:logger]` filter documented in the workflow~~|~~Medium~~|~~Med~~|~~S~~|~~45m~~|~~rest~~|
|~~M13~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Coverage floor (OWNER decision):** accept a floor (today: root 100 / server 98.6 / client 99.1 / entitytag 98.9 / metrics 96.8) and gate it in CI, or drop the idea explicitly~~|~~Low~~|~~Low~~|~~S~~|~~30m~~|~~rest~~|
|~~M14~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Dependency sanity:** gosec note on `Code`/`Domain` template-injection surface; read the go-error-family v0.10.0→v0.10.1 diff (bumped blind at `7ae7501`)~~|~~Low~~|~~Low~~|~~S~~|~~30m~~|~~rest~~|
|~~M15~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**httputil template parity:** verify httputil's suite pins go-etag's `errorTemplates` values verbatim (verify-before-filing); upstream a mirror test if absent; cross-link AGENTS error sections both repos~~|~~Medium~~|~~Med~~|~~S~~|~~45m~~|~~rest~~|
|~~M16~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Fuzz expansion:** stored-validator property (symmetric `weaklyMatches`; unparseable stored ⇒ false), `mergeHeader`, Cache-Control directive variants — mirror `FuzzHasNoStoreDirective`'s seed+soundness pattern; wire seeds into the CI fuzz job~~|~~Medium~~|~~Med~~|~~M~~|~~75m~~|~~rest~~|
|~~M17~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Spec pin-ups:** request-side `no-cache` (§5.2.2.2), HEAD-freshening × `resp.Uncompressed` interplay, `restoreMismatchedValidator` restricted-mode+dual-key edge — each citing its RFC section~~|~~Medium~~|~~Med~~|~~M~~|~~60m~~|~~rest~~|
|~~M18~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Post-release verification:** link-check `docs/rfc9111-conformance.md` Interpretation section; pkg.go.dev renders `metrics` + the four package READMEs at v0.5.0~~|~~Low~~|~~Low~~|~~S~~|~~30m~~|~~rest~~|
|~~M19~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Process hardening:** numbered-item detector before archive moves (`grep -n '^\s*\d\+\.\s\*\*' <f>~~|~~grep -v '~~'`);`annotate-rows.py --dry-run` trial on one annotated file; before→after scoring rule for audit sessions — recorded in AGENTS.md where sessions read them~~|~~Medium~~|~~Med~~|~~S~~|~~45m~~|
|~~M20~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Micro-policies in AGENTS.md:** docs-only sessions ⇒ no CHANGELOG entry; check-rows PARTIAL = house style for task tables; status-archive convention (once M1 answers g.3)~~|~~Low~~|~~Med~~|~~S~~|~~30m~~|~~rest~~|
|~~M21~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**README sections:** "Middleware Chaining" (compose with logging/CORS/recovery), "Troubleshooting" (no ETag on POST, 304 not returned, buffer overflow), CDN/proxy ETag-stripping note~~|~~Low~~|~~Med~~|~~M~~|~~60m~~|~~rest~~|
|~~M22~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**CI polish:** `workflow_dispatch` trigger for probes; LICENSE/README drift check; decide dprint-in-CI vs "daemon is the formatter of record"~~|~~Low~~|~~Low~~|~~S~~|~~45m~~|~~rest~~|
|~~M23~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Repo hygiene:** archive/trash the stray `workflow-audit-log-20260911-*.html` at root; branch protection on master (OWNER); homepage URL → pkg.go.dev (OWNER)~~|~~Low~~|~~Low~~|~~S~~|~~30m~~|~~rest~~|
|~~M24~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**ROADMAP Theme 1 design doc:** opt-in §4.2 freshness serving (`max-age`/`Expires` parsing, serve-within-freshness, default stays accelerator), incl. spec-test plan and conformance-table impact~~|~~High~~|~~High~~|~~L~~|~~100m~~|~~roadmap~~|
|~~M25~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**ROADMAP Theme 3 spike:** client hooks `OnHit`/`OnStore`/`OnFreshen`/`OnInvalidate` (mirror server hooks; complements `metrics/`) — design + demand note~~|~~Medium~~|~~Med~~|~~L~~|~~100m~~|~~roadmap~~|
|~~M26~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**v1.0.0 criteria + shim-removal checklist:** what gates the major besides shim deletion; the deletion checklist (`deprecated.go`, root `doc.go`, `deprecated_test.go`, docs sweep, consumer audit)~~|~~Medium~~|~~Med~~|~~M~~|~~60m~~|~~roadmap~~|
|~~M27~~ carried — superseded by `docs/planning/2026-09-23_14-07` (same M-number); now `TODO_LIST.md`|~~**Public presence spike:** comparison table vs other Go ETag/caching libraries, awesome-go submission draft, website-launch evaluation (Theme 4)~~|~~Low~~|~~Med~~|~~M~~|~~60m~~|~~roadmap~~|
