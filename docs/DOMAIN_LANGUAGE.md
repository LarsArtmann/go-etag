...
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Entity tag (ETag)   | An opaque identifier for a specific version of a representation (RFC 9110 §8.8.3): `W/"abc123"` or `"abc123"`.                                                                                                                               | `entitytag/entity_tag.go` (`ETag`; re-exported by `server/entity_tag.go`) |
| Validator           | A cache validator in general — an entity tag or a last-modified date — used by conditional requests. The client stores one per cache entry.                                                                                                  | `client/cache.go` (`storedValidator`)                 |
| Strength            | Whether an entity tag is `Strong` or `Weak`. Strong tags must change when the representation changes; weak tags may not. Encoded in the type, not a bool.                                                                                    | `entitytag/entity_tag.go` (`Strength`)                |
...
| Freshening          | RFC 9111 §4.3.4/§3.2: every header field a 304 provides replaces the stored value (except hop-by-hop, Content-Length/Content-Range, and Content-Encoding of a decoded body). The default (`FreshenPerRFC()`, the `FreshenOn304` zero value). | `freshenedHeader`, `persistFreshened`                 |
...
| Stored response     | The client's cache entry: status, headers, body, and a `storedValidator` (the wire string replayed verbatim as `If-None-Match`, plus the parsed `entitytag.ETag` built once at store time — the two can never disagree).                    | `client/cache.go` (`storedResponse`)                  |
| Error code (`Code`) | The typed construction surface for classified errors: six family constructors (Rejection/Conflict/Transient/Corruption/Infrastructure/Orchestration) plus matching `Wrap*` methods; mirrors httputil's pattern.                              | `server/code.go`                                      |
| Domain              | The failing-component prefix of an error code (`http.etag_write_failed` → `http`) — lets consumers route classified errors by component without string parsing.                                                                             | `DomainOf`, `InDomain` (`server/code.go`)             |
