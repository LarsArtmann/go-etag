...
|---|---|---|---|---|
|~~1~~ done — OQ8 (owner: enforce): gate step + baseline file `reports/dupl/2026-09-23_art-dupl-baseline.txt` (`ab1bcec`, report 16:40 a7)|~~Decide enforcement policy for the accepted clone (see g.1), then implement it (baseline file or CI assert "exactly 1 group")~~|~~High~~|~~M~~|~~Quality~~|
...
|~~7~~ done — `reports/bench/2026-09-23_{before,after}-head-freshening.txt` (report 04-52 §b.5)|~~Benchmark the HEAD freshening path (extra `header.Clone()`) per the reports/bench discipline~~|~~Low~~|~~M~~|~~Quality~~|
|~~8~~ done — nil branch proved unreachable; `TestCloneHeaderNilReturnsEmpty` `8c1c1a7` (report 04-52 §b.4)|~~Prove-or-delete `cloneHeader`'s nil branch (unit test or drop the guard)~~|~~Medium~~|~~S~~|~~Quality~~|
|~~9~~ done — both at 100% (report 04-52 a4)|~~Coverage check on `persistFreshened` / `cloneHeader` lines (`go test -coverprofile`)~~|~~Medium~~|~~S~~|~~Quality~~|
|~~10~~ superseded — live truth captured into the baseline file + gate count check (report 16:40 a7)|~~Verify `art-dupl` supports a baseline/check subcommand before building item 1 on it (verify-external-claims)~~|~~Medium~~|~~S~~|~~Quality~~|
|~~11~~ done — OQ8: machine-enforced (report 16:40 a7; `ab1bcec`)|~~Decide the art-dupl CI question once and for all (threshold, baseline, or explicit Won't) — deferred since 2026-08-07~~|~~Medium~~|~~S~~|~~Process~~|
...
|~~13~~ done — fixed upstream 2026-09-22 (`crushrc` `--env GOTOOLCHAIN auto`; AGENTS Commands)|~~Evaluate fixing the LSP experience (gopls/golangci-lint-ls error on every file) via a devShell GOTOOLCHAIN override~~|~~Medium~~|~~M~~|~~Quality~~|
...
|~~15~~ dies per OQ5 — standing practice; revive on the next error-surface change|~~Run erraudit `--no-suppress` audit pass and re-triage surfaced findings (last done 09-11)~~|~~Low~~|~~S~~|~~Quality~~|
|~~16~~ dies per OQ5 — the -t 1 baseline captures the actionable truth|~~Run art-dupl at default threshold 5 as a regression sanity alongside -t 1~~|~~Low~~|~~S~~|~~Quality~~|
|~~17~~ done — resolved by the M9 re-baseline live truth (report 16:40 a7: 4 shown groups, none of this pair)|~~Decide non-type-aware `return tags` clone (ParseETagList vs splitRawETags): accept or refactor~~|~~Low~~|~~S~~|~~Quality~~|
...
|~~20~~ done — testableexamples linter green through every gate|~~Confirm every `Example*` function in the repo has a `// Output:` directive (testableexamples linter requirement)~~|~~Low~~|~~S~~|~~Quality~~|
|~~21~~ done — pinned in the v0.3.0 HEAD-freshening spec suite (AGENTS claim matches)|~~Verify a negative spec test exists: `FreshenOn304` does NOT apply to HEAD updates (AGENTS.md claim) — add if missing~~|~~Medium~~|~~S~~|~~Quality~~|
|~~22~~ done — Age monotonicity specs shipped v0.3.0 (CHANGELOG)|~~Verify spec coverage: Age never runs backwards on rebuilds — add pin if missing~~|~~Low~~|~~S~~|~~Quality~~|
|~~23~~ done — Uncompressed-carrying rebuilds shipped v0.3.0 (CHANGELOG; FEATURES row)|~~Verify spec coverage: rebuilt responses carry `Uncompressed` for decoded stored bodies — add pin if missing~~|~~Low~~|~~S~~|~~Quality~~|
|~~24~~ done — quoted-arg subtests in the no-store specs (FEATURES row; v0.3.0)|~~Verify spec coverage: quote-aware no-store parsing (`"no-store"` argument does not match) — fuzz target exists; check spec pin too~~|~~Low~~|~~S~~|~~Quality~~|
|~~25~~ done — `TestRoundTripSeparatesCredentialsViaKeyFunc` + doc warning (FEATURES row)|~~Verify default KeyFunc (= URL string) has a spec pin; KeyFunc credential warning lives only in client/doc.go~~|~~Low~~|~~S~~|~~Quality~~|
|~~26~~ done — `TestResponseCacheStats` (FEATURES row)|~~Verify Stats() (Hits/Stored/Entries) counters have direct spec assertions, not just incidental coverage~~|~~Low~~|~~S~~|~~Quality~~|
|~~27~~ dies per OQ5 — stub coverage exists; real-wire variant revive on demand|~~Consider an integration test for oversized-body streaming (buffered prefix chained to live remainder over a real server)~~|~~Low~~|~~M~~|~~Quality~~|
|~~28~~ dies per OQ5 — spec exists; real-wire variant revive on demand|~~Consider pinning 304 passthrough when the caller set their own If-None-Match over the real-wire server suite~~|~~Low~~|~~S~~|~~Quality~~|
|~~29~~ carried — `TODO_LIST.md` #2 (M15/F56: verified unpinned 2026-09-23; upstream decision pending)|~~Cross-repo: keep go-etag error-template wording byte-identical with httputil's registration (AGENTS.md warns of drift); schedule a parity check~~|~~Medium~~|~~M~~|~~Quality~~|
|~~30~~ superseded — policy machine-enforced (OQ8) + documented in AGENTS Non-Obvious Behaviors; ADR unnecessary|~~Consider an ADR in docs/decisions/ for the accepted-clone policy (discoverability beyond AGENTS.md)~~|~~Low~~|~~S~~|~~Documentation~~|
|~~31~~ superseded — the 06-18 snapshot's cite annotated as historical drift (this pass)|~~Annotate the 09-11 06:18 snapshot's stale line cite (testutil_test.go:57 → now 112-114) per snapshot-annotation policy~~|~~Low~~|~~S~~|~~Documentation~~|
|~~32~~ done — this pass verified the file's tables structurally (check-rows uniform)|~~Verify the daemon's 154-line reflow of `docs/status/2026-09-18_21-00_v040-release-session.md` (commit `969d077`) did not corrupt tables~~|~~Low~~|~~S~~|~~Quality~~|
|~~33~~ carried — `TODO_LIST.md` #4 (M18 micro-policies)|~~Confirm no CHANGELOG entry is needed for docs-only sessions (policy check)~~|~~Low~~|~~S~~|~~Process~~|
...
|~~35~~ done — standing discipline codified in AGENTS benchmark discipline|~~Capture a fresh `reports/bench` baseline (-benchmem -count=6) before the next perf-relevant change~~|~~Low~~|~~S~~|~~Process~~|
|~~36~~ done — runbook seeded in AGENTS (F80); full checklist carried `TODO_LIST.md` #4 (M24)|~~Draft the v1.0.0 shim-removal checklist (deprecated.go + root doc.go + deprecated_test.go)~~|~~Low~~|~~M~~|~~Cleanup~~|
|~~37~~ done — ROADMAP Parked updated with the architecture-unblock note (this pass)|~~Unpark criteria review for a go-etag/otel subpackage, post-v0.3.0 (old roadmap item)~~|~~Low~~|~~S~~|~~Feature~~|
|~~38~~ dies per OQ5 — practice is coherence-based; formalize on demand|~~Release-cadence rule for ROADMAP (old open item)~~|~~Low~~|~~S~~|~~Process~~|
|~~39~~ Won't implement — OQ5 (zero-binary library)|~~Cosign/SBOM evaluation for releases (old open item)~~|~~Low~~|~~M~~|~~Process~~|
|~~40~~ done — `ab1bcec` OQ3: `release.yml` live|~~Decide the release-workflow open question (old ROADMAP OQ3)~~|~~Low~~|~~S~~|~~Process~~|
|~~41~~ done — clean-room pattern documented in AGENTS consumer gates|~~TODO #27-carryover: retire/rotate the /tmp verification pattern into the repo (v0.4.0 report table)~~|~~Low~~|~~S~~|~~Cleanup~~|
|~~42~~ superseded — v0.5.0/v0.6.0 shipped with bench pairs; M11 regenerates comparisons|~~Re-run `go test -bench=. -count=1` smoke on server package to confirm no perf surprises since 09-18 typed-validator baselines~~|~~Low~~|~~S~~|~~Quality~~|
|~~43~~ done — fuzz-path fix + entitytag targets verified green through the v0.6.0 tag run (CHANGELOG 0.6.0)|~~Verify the CI fuzz job still wires both client fuzz target and server targets (AGENTS.md claim)~~|~~Low~~|~~S~~|~~Quality~~|
...
|~~45~~ done 2026-09-23 — AGENTS shared-tree protocol bullet|~~Consider making the session-start ritual explicit in AGENTS.md: read Repo Workflow Notes before touching shared files (daemon rewrites)~~|~~Low~~|~~S~~|~~Documentation~~|
|~~46~~ done — this pass re-verified the rot-prone claims (art-dupl groups, coverage, bench files) against live state|~~Audit whether any other doc comments in the repo state tool-output facts that can rot (grep for "art-dupl", "0 issues", "reports" in comments)~~|~~Low~~|~~S~~|~~Documentation~~|
...
|~~50~~ Won't implement here — belongs in the crush-config repo's references/lessons.md (cross-project lessons are committed there, never in-session)|~~Write the cross-project lesson "tool preconditions live in env, not in the command name (golangci-lint inherits GOTOOLCHAIN)" to references/lessons.md in crush-config (by commit, not in-session write)~~|~~Low~~|~~S~~|~~Process~~|
