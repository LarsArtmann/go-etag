...
| 11 | Curate the v1.0.0 GitHub Release page (currently `--generate-notes` raw output; house style is curated)                                                                                                                   | Low      | XS     | Release       |
| 12 | benchstat via sanctioned path; regenerate the three baselines as a comparison table (pre-existing)                                                                                                                        | Medium   | S      | Quality       |
| 13 | Record accepted coverage floor (98.3%) as explicit decision (pre-existing)                                                                                                                                                | Medium   | XS     | Quality       |
| 14 | `Freshened`/`Invalidated` counters on `Stats` (pre-existing)                                                                                                                                                              | Medium   | S      | Feature       |
| 15 | Explicit subtest: caller-conditional HEAD receiving 304 passes through (pre-existing)                                                                                                                                     | Medium   | S      | Quality       |
| 16 | HEAD-freshening × `resp.Uncompressed` interplay test (pre-existing)                                                                                                                                                       | Medium   | S      | Quality       |
| 17 | GoDoc examples for `FreshenPerRFC`/`FreshenNone` (or one policy-selection example) — FreshenPolicy coverage is partial                                                                                                    | Low      | S      | Documentation |
| 18 | Rebase/fix the red dependabot actions-group PR in httputil (was red pre-session; lint-pin fixes may unblock it)                                                                                                           | Low      | S      | Maintenance   |
| 19 | go directive floor policy: cqrs-htmx modules sit at `go 1.26.7` (patch floor) vs the skill's major.minor rule — decide once                                                                                               | Low      | S      | Policy        |
| 20 | Consider `GOTOOLCHAIN` job-scoping audit for httputil/cqrs-htmx CI (same hazard go-etag just fixed)                                                                                                                       | Low      | S      | Bug           |
| 21 | cqrs-htmx: verify `go.work.sum` cleanliness post-churn (currently pass-by-build; never explicitly inspected)                                                                                                              | Low      | XS     | Maintenance   |
