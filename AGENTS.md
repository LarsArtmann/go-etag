...
**Known tool noise (do not "fix"):** golangci-lint warns `unknown linters in //nolint directives: erraudit` — benign, because the directives serve the standalone erraudit CLI (module-scoped, run separately above), not golangci-lint; do not delete `//nolint:erraudit` directives over this warning (they are re-validated by the nolint audit). The standalone CLI's `--explain` flag is deliberately not used in these examples — the gate runs without it (recorded 2026-09-23, M19/F73).

**Verification gate (first pass, not last):** `golangci-lint fmt` (root + per module) → `golangci-lint run` root + per nested module (0 issues) → `go vet` over `$PACKAGES` → `go test -race -count=1` over `$PACKAGES` — even for comment-only edits (godot/wsl bite comments too; all commands take the `GOTOOLCHAIN=auto` prefix). `scripts/pre-release-check.sh` encodes the whole loop. Supplement when touching clones or error paths: `art-dupl -t 1 --type-aware` (expect exactly the 4 accepted groups, see Non-Obvious Behaviors).
...
- **The four `art-dupl` clone groups are accepted — do not extract them** — `art-dupl -t 1 --type-aware` shows 4 groups (baseline: `reports/dupl/2026-09-23_art-dupl-baseline.txt`): (1) `etagWriter.Write`'s flushed-write vs overflow-write blocks in `server/etag.go` — both delegate then classify with deliberately distinct error context ("streaming write failed" vs "overflow write failed"), a parameterized helper would hide the branch semantics the messages exist to convey; (2) the §4.3.5 HEAD-confirmation guard arms in `client/transport.go` (`return false` on ETag mismatch, `return false` on Last-Modified mismatch) — two distinct RFC rules, not duplication; (3) the read-error and oversized-body arms both chaining `resp.Body = newChainedBody(buffered, resp.Body)` — the shared logic is a single line, a helper would take more parameters than the duplication has lines; (4) the `default: return false` arms of the §3.2 field-filter and §3.1 hop-by-hop switches — idiom, not duplication. The historical fifth pair (the `Write` body vs `nonHijackableRecorder.Write` in `server/testutil_test.go`) is now filter-suppressed (test files) but the do-not-extract rule stands: the test double must NOT implement `http.Hijacker` (that is its entire testing purpose), so it cannot embed `httptest.ResponseRecorder` or share any production buffer type. Accept all of these in every future dedup pass — enforced, not just documented: the release gate fails unless exactly 4 groups are shown (OQ8, 2026-09-23).
...
- **An auto-git daemon commits and re-formats continuously** — status reports and tables get re-touched within minutes of your edit. Always re-read a shared file immediately before re-editing it; never assume remembered file geometry. Observed daemon behaviors: it rewrote the `go 1.27.1` directive to `go 1.27` twice (`969d077`, `6f15f81` — the release-gate parity check catches this class), and it regenerates the buildflow-managed `.gitignore` block, re-adding `go.work`/`go.work.sum` ignores that were removed by hand (2026-09-23: this silently untracked `go.work` and cost a fully red CI run — the durable fix is `!go.work`/`!go.work.sum` negation lines OUTSIDE the marker block, where regeneration cannot touch them).
- **Shared-tree protocol (multiple agent sessions may run in this repo at once)** — check `git log` + `git status` (and the remote) before deep work and again before any edit batch; when a release train is in flight, go docs-only or nothing. Both 2026-09-23 lanes independently worked blind to each other for ~15 tool calls (reports 15:35 d1, 16:40 d1); a start-of-session git-state check would have exposed the parallel lane immediately.
...
Everything else carries over: tag only after CI green on the exact commit, load the go-release skill, pkg.go.dev is not a gate. The GitHub Release page is automated since OQ3 (2026-09-23): pushing the ROOT `v*` tag triggers `.github/workflows/release.yml`, which extracts the matching CHANGELOG section as the notes (it fails loudly if the section was not cut) and creates the Release as Latest, idempotently; nested tags get no Release pages. Keep the CHANGELOG section format stable — the extractor matches the section heading by prefix, so restructured headings break notes extraction. The workflow has NOT fired live yet (verified locally only: extraction, missing-section failure path, actionlint); its first real run is the next root tag — watch it end to end (untested-by-default honesty per report 22:07 b4/e4). The proposal doc `docs/modularization/2026-09-23_modularization-proposal.html` records why metrics got a module (tombstone purity, not composability) and why go.mod replace directives stay banned (proxy-published repo).

**v1.0.0 root-module deletion mini-staircase (seed, executed at the major):** the v1.0.0 release is itself a small staircase — (1) delete `deprecated.go`/`doc.go`/root tests and the root MODULE (its go.mod/go.sum), drop the root entry from go.work's `use` + replaces, and remove the root pattern from every PACKAGES set (ci.yml + pre-release-check.sh — the agreement check now enforces they stay identical); (2) re-run the bottom-up staircase for the FOUR surviving modules at `v1.0.0` exactly as above; (3) the v1.0.0 GitHub Release notes carry the root-consumer migration (pure `go get …/server` import-path swap). Consumers importing the bare root path break loudly at `go mod tidy` — that is the intended forcing function; audit the fleet first with `rg -l "larsartmann/go-etag\" v" -g go.mod ~/projects` (dead requires at v0.x can simply drop the line).
